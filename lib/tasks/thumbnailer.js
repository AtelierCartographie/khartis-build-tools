...
Thumbnailer.prototype.copyToSrc = function(file, fileName) {
  let base = process.cwd();
  fs.ensureDirSync(`${base}/public/assets/images/map-thumbnails`);
  return new Promise( (res, rej) => {
    fs.copy(file, `${base}/public/assets/images/map-thumbnails/${fileName}`, function(err) {
      if (err) rej(err);
      res(fileName);
    });
  });
            ...
            .then(mtn => {
              return page.$$(".map-editor").then(els => {
                return Promise.all(els.map((el, i) => {
                  return elBbox(el)
                    .then(bbox => {
                      let fileName = `${mtn[i].id.replace(/\s/g, '_')}.png`,
                          distPath = `${outputPath}/assets/images/map-thumbnails/${fileName}`;
                      return page.screenshot({ path: distPath, clip: bbox })
                        .then( () => this.copyToSrc(distPath, fileName) );
                    })
